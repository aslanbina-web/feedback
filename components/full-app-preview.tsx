...
function Home({ go }: { go: Go }) {
  return <>
    ...
    <div className="daily-stats"><div className="daily-stat give"><span className="daily-symbol"><UiIcon name="send" /></span><span>Gives Today</span><strong>1 / 3</strong></div><div className="daily-stat receive"><span className="daily-symbol"><UiIcon name="heart" /></span><span>Receives Today</span><strong>2 / 3</strong></div></div>
...
function Discover({ go }: { go: Go }) {
  return <><h1 className="page-title compact-title">Your Assigned Matches.</h1><p className="lede blue">Up to 3 available today.</p><div className="assignment-refresh"><UiIcon name="refresh" /> New assignments in 29:18</div><div className="match-list">{matches.map((match) => <a className="match-card" href={previewHref("accept")} key={match.label}><Image className="match-image" src={getBusinessCategory(match.category).image} alt="" width={120} height={110} /><div className="match-copy"><span className="category-pill">{match.label}</span><strong><UiIcon name="pin" />{match.city} · {match.district}</strong><p>{match.description}</p></div><span className="match-arrow">›</span></a>)}<div className="discover-privacy"><span aria-hidden="true"><UiIcon name="lock" /></span><p>Business name, owner and review link stay hidden until you accept.</p></div></div></>;
}
...
function Done({ go }: { go: Go }) {
  return <div className="done-screen"><div className="celebration-wrap"><UiIcon name="party" /><Image className="celebration-star" src="/giveget-star.svg" alt="Celebrating GiveGet star" width={190} height={175} /><UiIcon name="star" /></div><h1 className="page-title">Great job!</h1><p className="lede blue">Your review is complete!</p><div className="credit-earned"><UiIcon name="star" /><strong>+1 credit added</strong></div><a className="button green full" href={previewHref("discover")}>Find Another Business</a><a className="button white full" href={previewHref("history")}>View History</a><p className="history-save-note"><UiIcon name="party" /> Your completed review and its URL are saved in History.</p></div>;
}
...
function History() {
  ...
  return <><h1 className="page-title compact-title">History.</h1><p className="lede blue">Your completed and expired matches.</p><div className="tabs"><button className={tab === "gives" ? "active" : ""} onClick={() => setTab("gives")}>My Gives</button><button className={tab === "receives" ? "active" : ""} onClick={() => setTab("receives")}>My Receives</button></div><div className="history-list">{rows.map((row, index) => <article key={row[0] + row[1]}><div className={`history-art history-art-${index + 1}`} aria-hidden="true" /><div className="history-copy"><h2>{row[0]}</h2><div className="history-date">{row[1]} · Taichung</div><span className={row[2] === "Expired" ? "history-status expired" : "history-status"}>{row[2]}</span>{row[2] === "Completed" ? <a href="https://maps.google.com" target="_blank" rel="noreferrer">View review URL ↗</a> : null}</div><strong className="history-points">{row[2] === "Completed" ? <>{tab === "gives" ? "+1 " : null}<UiIcon name="star" /></> : <UiIcon name="clock" />}</strong></article>)}</div><aside className="history-save-note"><span><UiIcon name="external" /></span><span>Completed review links stay available here for future reference.</span></aside></>;
}

function Profile({ go }: { go: Go }) {
  return <><section className="profile-identity"><div className="avatar">M</div><div><strong>Mina Chen</strong><span><UiIcon name="sprout" /> Free Plan</span></div></section><section className="business-card-panel"><div className="profile-title-row"><h1 className="page-title compact-title">My Card.</h1><a className="profile-edit-button" href={previewHref("edit")}><UiIcon name="edit" /> Edit</a></div><article className="business-summary"><div className="profile-business-art" role="img" aria-label="Mina's Flower Studio" /><div><h2>Mina&apos;s Flower Studio</h2><span>Florist · Taichung · Nantun</span><p>A small floral studio creating seasonal bouquets and thoughtful gifts.</p></div></article><a className="sample-action-link" href={previewHref("samples")}><UiIcon name="edit" /> Add Sample Reviews</a><p className="sample-helper">Add sample reviews you prefer for your shop.<br /><strong>8 samples available</strong></p></section><div className="quota-stats"><div><span>Credits</span><strong><UiIcon name="star" /> 4</strong></div><div><span>Gives</span><strong className="green-text">12 / 30</strong><small>22 days left</small></div><div><span>Receives</span><strong className="red-text">8 / 30</strong><small>22 days left</small></div></div><p className="saved-passes">Saved passes never expire.</p></>;
}
...
function Samples({ go }: { go: Go }) {
  ...
  return <><h1 className="page-title compact-title">Sample Reviews.</h1><p className="lede blue">Add review wording you genuinely prefer customers to use as inspiration.</p><form className="sample-add-card" onSubmit={(event) => { event.preventDefault(); if (!draft.trim()) return; setSamples((current) => [...current, draft.trim()]); setDraft(""); }}><label htmlFor="preview-sample">Add a new sample</label><textarea id="preview-sample" value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={500} placeholder="Example: Friendly service and beautiful fresh flowers..." /><div className="sample-counter">{draft.length} / 500</div><button className="button green full">Add to My Samples ✦</button></form><div className="sample-list-heading"><h2>Your samples</h2><span>{samples.length + 6} available</span></div><div className="managed-sample-list">{samples.map((text, index) => <article key={text + index}><p>{text}</p><button aria-label="Delete sample" onClick={() => setSamples((all) => all.filter((_, item) => item !== index))}><UiIcon name="trash" /></button></article>)}</div><aside className="sample-pool-note"><UiIcon name="star" /> When a giver accepts your card, one sample is chosen at random and removed from this available list.</aside></>;
}

function Invite() {
  const [copied, setCopied] = useState(false);
  return <><div className="invite-title"><div><h1 className="page-title compact-title">Invite &amp; Grow.</h1><span className="plan-badge"><UiIcon name="sprout" /> Free Plan</span><p className="lede blue">Invite another business owner and earn 30 more free days.</p></div><Image src="/giveget-invite-star.webp" alt="GiveGet star" width={100} height={92} /></div><section className="referral-card"><h2>Your Referral Link</h2><div><code>giveget.app/r/mina12</code><button onClick={() => setCopied(true)}>{copied ? "Copied!" : "Copy"}</button></div><div className="share-icons"><button><span className="share-mark">LINE</span>LINE</button><button><span className="share-mark">WA</span>WhatsApp</button><button><span><UiIcon name="mail" /></span>Email</button><button><span><UiIcon name="more" /></span>More</button></div></section><section className="how-it-works"><h2>How it works</h2><ol><li>Invite a new business through your unique link.</li><li>They create their business card.</li><li>They add at least one sample review.</li><li>They complete their first Give.</li></ol></section><aside className="referral-reward"><strong><UiIcon name="party" /> You receive +30 free days</strong><br />Only after all four steps are completed.<br />The reward applies to the inviter only.</aside></>;
}

function Notifications({ go }: { go: Go }) { const items = [["clock","Your task expires soon","Bloom & Co. has 18 minutes remaining. Resume your review to keep the credit chance.","Just now"],["mail","New matches are ready","You have two assigned business cards available today.","10 minutes ago"],["sprout","Free plan: 3 days left","Invite a business friend to earn another 30 free days after they qualify.","Today"],["party","Referral qualified!","Your invited business completed its first Give. We added 30 free days.","Yesterday"]] as const; return <div className="notification-list">{items.map((item) => <article key={item[1]}><span className="notice-art"><UiIcon name={item[0]} /></span><div><strong>{item[1]}</strong><p>{item[2]}</p><time>{item[3]}</time></div></article>)}</div>; }
function Settings({ go }: { go: Go }) { const rows = [["profile","Account","LINE identity and profile","profile"],["bell","Notifications","Task, match and plan alerts","notifications"],["shield","Privacy & Safety","Community rules and reports",null],["help","Help","How GiveGet works",null]] as const; return <><section className="settings-list">{rows.map((row) => { const content = <><span className="setting-art"><UiIcon name={row[0]} /></span><div><strong>{row[1]}</strong><small>{row[2]}</small></div><b>›</b></>; return row[3] ? <a className="setting-row" href={previewHref(row[3])} key={row[1]}>{content}</a> : <div className="setting-row" key={row[1]}>{content}</div>; })}</section><section className="settings-list"><a className="setting-row" href={previewHref("invite")}><span className="setting-art"><UiIcon name="sprout" /></span><div><strong>Free Plan</strong><small>22 days remaining</small></div><b>›</b></a><a className="setting-row" href={previewHref("invite")}><span className="setting-art"><UiIcon name="mail" /></span><div><strong>Invite &amp; Grow</strong><small>Earn another 30 free days</small></div><b>›</b></a></section><button className="settings-logout">Log out</button><p className="version-note">GiveGet version 1.0</p></>; }

const navItems = [{ screen: "home", label: "Home", icon: "home" }, { screen: "discover", label: "Discover", icon: "search" }, { screen: "tasks", label: "Tasks", icon: "tasks" }, { screen: "history", label: "History", icon: "history" }, { screen: "profile", label: "Profile", icon: "profile" }] as const;
...
export function FullAppPreview({ initialScreen = "home" }: { initialScreen?: string }) {
  ...
  return <main className={`shell board-shell screen-${screen}`}><header className="masthead">{showBack ? <a className="back-button" href={previewHref(backScreen)} aria-label="Go back"><UiIcon name="back" /></a> : null}{headerTitle === "GiveGet" ? <a className="brand preview-link" href={previewHref("home")}>GiveGet</a> : <span className="brand">{headerTitle}</span>}<div className="header-actions">{screen === "home" || screen === "tasks" ? <a className="header-icon" href={previewHref("notifications")} aria-label="Notifications"><UiIcon name="bell" /></a> : screen === "history" ? <div className="credits"><UiIcon name="star" /><strong>4</strong></div> : screen === "profile" ? <a className="header-icon profile-settings-icon" href={previewHref("settings")} aria-label="Settings"><UiIcon name="settings" /></a> : screen === "samples" || screen === "invite" || screen === "notifications" || screen === "settings" || screen === "edit" ? <span className="sample-header-spacer" aria-hidden="true" /> : <div className="credits"><UiIcon name="star" /><strong>4</strong></div>}</div></header><div className="content">{content}</div>{screen !== "edit" ? <nav className="nav preview-nav">{navItems.map((item) => <a key={item.screen} href={previewHref(item.screen)} className={screen === item.screen || (profileChild && item.screen === "profile") ? "active" : ""}><span className="nav-bubble" aria-hidden="true"><UiIcon name={item.icon} /></span>{item.label}</a>)}</nav> : null}</main>;
}
